...
FINITA_LA_COMEDIA:
			...
			if (A_ThisLabel = "FINITA_LA_COMEDIA")
			{
				if (A_IsCompiled = 1)
					...
					msgbox, 262144, CompanyChecker - Cryption №1, Error №%err2%№%err%`nОшибка расшифровки данных
		...
		FINITA_DA_COMEDIA:
			...
			if (A_ThisLabel = "FINITA_LA_COMEDIA")
			{
				if (A_IsCompiled = 1)
					...
					msgbox, 262144, CompanyChecker - Cryption №2, Error №%err2%№%err%`nОшибка расшифровки данных
					;msgbox % foo " call failed with:`nErrorLevel: " err2 "`nLastError: " err "`n" ErrorFormat(err) 
				return 0
			}
			return hashval
...
SetKeySalt(hKey,hProv)
{
	...
	if !dllCall("Advapi32\CryptSetKeyParam"
				...
				,"Ptr",&st
				,"Uint",0)
		msgbox, 262144, CompanyChecker - Cryption №3, Error №%err2%№%err%`nОшибка расшифровки данных
...
GetKeySalt(hKey)
{
	...
	if !dllCall("Advapi32\CryptGetKeyParam"
				...
				,"Uint*",dwCount
				,"Uint",0)
	msgbox, 262144, CompanyChecker - Cryption №4, Error №%err2%№%dwCount%№%err%`nОшибка расшифровки данных
	...
	if !dllCall("Advapi32\CryptGetKeyParam"
				...
				,"Uint*",dwCount
				,"Uint",0)
	msgbox, 262144, CompanyChecker - Cryption №5, Error №%err2%№%dwCount%№%err%`nОшибка расшифровки данных
